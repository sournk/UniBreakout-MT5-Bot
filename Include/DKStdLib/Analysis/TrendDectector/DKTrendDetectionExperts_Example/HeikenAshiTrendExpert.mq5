//=====================================================================
//	Эксперт на индикаторе тренда HeikenAshiTrendDetector.
//=====================================================================
#property copyright 	"Dima S."
#property link      	"dimascub@mail.com"
#property version   	"1.00"
#property description "Эксперт на индикаторе тренда HeikenAshiTrendDetector."
//---------------------------------------------------------------------
...
//	Создадим хэндл внешнего индикатора для дальнейшего обращения к нему:
   ResetLastError();
   indicator_handle=iCustom(Symbol(),PERIOD_CURRENT,"Examples\\HeikenAshiTrendDetector");

//	Если инициализация прошла неудачно, то возвратим ненулевой код:
   if(indicator_handle==INVALID_HANDLE)
     {
      Print("Ошибка инициализации HeikenAshiTrendDetector, Код = ",GetLastError());
      return(-1);
     }
   return(0);
  }
...
//	Получаем сигнал из индикатора тренда:
   ...
   return(( int)trend_direction[LEN-2]);
  }
//---------------------------------------------------------------------
